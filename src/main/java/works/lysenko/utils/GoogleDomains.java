...
/**
 * Weighted selector of local Country Google domain
 *
 * @author Sergii Lysenko
 */
@SuppressWarnings({"unused", "MagicNumber", "UtilityClass", "FinalClass", "AutoBoxing", "StaticCollection", "ClassWithoutLogger", "PublicMethodWithoutLogging", "StandardVariableNames", "ClassIndependentOfModule"})
public final class GoogleDomains {
    ...
    static {
        ...
        d.add(pair("google.hm", 0.0)); // not available, whois lookup not working
    ...
    /**
     * Select random Country Google domain. Probabilities for different countries are
     * defined in {@code static} section below
     *
     * @return randomly selected Google domain
     */
    @SuppressWarnings({"unused", "ChainedMethodCall"})
    public static String getGoogleDomain() {
        return new EnumeratedDistribution<>(d).sample();
    }

    @SuppressWarnings("QuestionableName")
    private static Pair<String, Double> pair(String string, Double pair) {
        return new Pair<>(string, pair);
    }
}
